...
def chat_panel_module(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules, "src.frontend.api_client.client", fake_client_module
    )
...
def test_initial_message_uses_local_fallback_when_backend_unavailable(
    chat_panel_module,
):
    class Backend:
        def start_chat(self, payload):
...
def test_send_budget_appends_user_message(chat_panel_module):
    ...
    chat_panel_module._send_message(
        "$450 per month", "sess-1", backend_client=Backend()
    )
...
def test_post_chat_preferences_are_written_to_state(chat_panel_module):
    ...
    chat_panel_module._send_message(
        "Budget is 500 and petrol", "sess-1", backend_client=Backend()
    )
...
def test_send_message_uses_backend_reply_and_quick_replies(chat_panel_module):
    class Backend:
        def post_chat(self, payload):
            return {
                "reply": "What body type do you want?",
                "quick_replies": ["SUV", "Hatchback"],
            }

    chat_panel_module._send_message(
        "My budget is 500", "sess-1", backend_client=Backend()
    )
...
@pytest.mark.parametrize(
    ("response", "expected_seconds"),
    [
        ({}, 0.0),
        ({"question_delay_ms": None}, 0.0),
        ({"question_delay_ms": 0}, 0.0),
        ({"question_delay_ms": -100}, 0.0),
        ({"question_delay_ms": "2500"}, 0.0),
        ({"question_delay_ms": 2750}, 2.75),
    ],
)
def test_response_question_delay_seconds_handles_delay_metadata(
    chat_panel_module, response, expected_seconds
):
    assert (
        chat_panel_module._response_question_delay_seconds(response) == expected_seconds
    )


def test_send_message_waits_for_backend_question_delay(chat_panel_module, monkeypatch):
    slept = []
    ...
    class Backend:
        def post_chat(self, payload):
            return {
                "reply": "What fuel type do you prefer?",
                "question_delay_ms": 2500,
                "target_slot": "fuel_type",
            }

    monkeypatch.setattr(
        chat_panel_module.time, "sleep", lambda seconds: slept.append(seconds)
    )

    chat_panel_module._send_message("I need a car", "sess-1", backend_client=Backend())

    assert slept == [2.5]
    assert (
        chat_panel_module.st.session_state["chat_messages"][-1]["text"]
        == "What fuel type do you prefer?"
    )


def test_send_message_does_not_wait_without_question_delay(
    chat_panel_module, monkeypatch
):
    slept = []
    ...
    chat_panel_module._send_message(
        "Yes, show recommendations", "sess-1", backend_client=Backend()
    )
...
def test_present_recommendations_action_does_not_append_summary_prompt(
    chat_panel_module,
):
